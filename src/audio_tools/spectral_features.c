    ...
    #pragma omp parallel
    {
        ...
        if (thread_fft->plan) {
            ...
            for (size_t i = 0; i < max_i; i++) {
                ...
                const size_t i_freq = i * num_frequencies;
                for (size_t j = 0; j < num_frequencies; j++) {
                    const float real = thread_fft->out[j][0];
                    const float imag = thread_fft->out[j][1];

                    result.magnitudes[i_freq + j] = sqrtf(real * real + imag * imag);
                    //result.phases[i_freq + j]     = atan2(imag,real); memory writes are costly
                }
            }
        }
    }
